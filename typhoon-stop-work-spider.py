...
class TyphoonJson(object):
    def __init__(self, updated, disaster_declarations):
        self.updated = updated
        self.disaster_declarations = disaster_declarations

class TyphoonMessage(object):
    def __init__(self, city, content):
        self.city = city
...
class TyphoonStopWorkSpider(scrapy.Spider):
    ...
    KAFKA_TOPIC = 'typhon_dayoff_declare'
...
#        producer = KafkaProducer(bootstrap_servers=['59.127.187.54:9092'])
        future = producer.send('typhon_dayoff_declare', typhoonJsonObj)
